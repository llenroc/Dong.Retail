...
using Dong.Retail.Tests.TestDatas;
...
namespace Dong.Retail.Tests
{
    ...
    /// Provides methods to easily work with <see cref="RetailDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<RetailTestModule>
    {
        ...
        {
            void NormalizeDbContext(RetailDbContext context)
            {
        ...
        protected void UsingDbContext(Action<RetailDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<RetailDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<RetailDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<RetailDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<RetailDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<RetailDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<RetailDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<RetailDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<RetailDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<RetailDbContext, Task<T>> func)
        {
            T result;
